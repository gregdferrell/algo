...
def test_find_second_largest_item_bst_traverse_left_subtree():
	root_node = BinaryTreeNode(10, None)
	new_node = root_node.insert_left(5)
	new_node = new_node.insert_right(6)

	node_found = find_second_largest_item_bst(root_node)
	assert 6 == node_found.value


def test_find_second_largest_item_bst_no_parent_not_enough_items():
	root_node = BinaryTreeNode(10, None)

	with pytest.raises(ValueError) as e:
		find_second_largest_item_bst_no_parent(root_node)


def test_find_second_largest_item_bst_no_parent_traverse_right_no_left():
	root_node = BinaryTreeNode(10, None)
	new_node = root_node.insert_right(11)
	new_node = new_node.insert_right(12)

	node_found = find_second_largest_item_bst_no_parent(root_node)
	assert 11 == node_found.value


def test_find_second_largest_item_bst_no_parent_traverse_right_left_subtree():
	root_node = BinaryTreeNode(10, None)
	new_node = root_node.insert_right(11)
	new_node = new_node.insert_right(12)
	new_node = new_node.insert_right(20)
...
def test_find_second_largest_item_bst_no_parent_traverse_left_subtree():
	root_node = BinaryTreeNode(10, None)
	new_node = root_node.insert_left(5)
	new_node = new_node.insert_right(6)
